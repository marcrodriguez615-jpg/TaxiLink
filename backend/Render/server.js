...
app.get('/google-ads/connect', (req, res) => {
  ...
  if (missing.length) {
    return res.status(500).send(`Google Ads no está configurado todavía. Faltan variables en Render: ${missing.join(', ')}`);
  }
...
app.get('/google-ads/oauth2callback', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok || token.error) {
      return res.status(400).send(token.error_description || token.error || 'No se pudo conectar Google Ads');
    }
    ...
    res.send('Google Ads conectado correctamente. Vuelve a TaxiLink y pulsa Actualizar datos reales.');
  } catch (error) {
...
app.get('/google-ads/summary', async (req, res) => {
  const missing = missingGoogleAdsEnv();
  if (missing.length) return res.status(500).json({ error: `Faltan variables en Render: ${missing.join(', ')}` });
  ...
  try {
    ...
    const response = await fetch(`https://googleads.googleapis.com/v24/customers/${customerId}/googleAds:searchStream`, {
      ...
      body: JSON.stringify({
        query: 'SELECT metrics.clicks, metrics.phone_calls, metrics.impressions, metrics.cost_micros FROM campaign WHERE segments.date DURING LAST_30_DAYS'
      })
    });
...
app.get('/taxis', (req, res) => {
  const identifier = req.query.identifier;
  const now = Date.now();
  const visible = taxis.filter(t => !identifier || t.identifier === identifier).map(t => ({
    ...t,
    online: t.lastUpdate ? now - new Date(t.lastUpdate).getTime() < 45000 : false
  }));
...
app.post('/taxis/:number/location', (req, res) => {
  ...
  Object.assign(taxi, {
    driverName: req.body.driverName || taxi.driverName,
